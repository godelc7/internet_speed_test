...
"""
In aim of this script is to measure the internet speed using the speedtest.net API and save the
detailed results in a .csv file the home directory. This is a modified version of the code found in
https://github.com/sivel/speedtest-cli.

Project: Hobby project

File:    internetSpeedtest.py
...
# Begin import some stuff to handle Python 2 and Python 3
try:
    import json
except ImportError:
    try:
...
if PY32PLUS:
    etree_iter = El_tree.Element.iter
elif PY25PLUS:
...
def build_user_agent():
    ...
    ua_tuple = (
        ...
        '(KHTML, like Gecko)',
        'internet_speed_tester/%s' % __version__
    )
...
def get_attributes_by_tag_name(local_dom, tag_name):
    ...
    """
    elem = local_dom.getElementsByTagName(tag_name)[0]
...
class Speedtest(object):
    ...
    def get_config(self):
        ...
        try:
            try:
                root = El_tree.fromstring(configxml)
        ...
        for url in urls:
            try:
                ...
                try:
                    try:
                        try:
                            root = El_tree.fromstring(serversxml)
                        except El_tree.ParseError:
                            e = get_exception()
                ...
                for server in elements:
                    ...
                    try:
                        d = compute_distance(self.lat_lon,
    ...
    def download(self, callback=do_nothing, threads=None):
        ...
        def producer(q_, requests_, request_count_):
            for i_, request in enumerate(requests_):
                ...
                q_.put(thread, True)
                in_flight['threads'] += 1
                callback(i_, request_count_, start=True)

        finished = []

        def consumer(q_, request_count_):
            _is_alive_ = thread_is_alive
            while len(finished) < request_count_:
                thread = q_.get(True)
                while _is_alive_(thread):
    ...
    def upload(self, callback=do_nothing, pre_allocate=True, threads=None):
        ...
        def producer(q_, requests_, request_count_):
            for i_, request in enumerate(requests_[:request_count_]):
                thread = HTTPUploader(
                    i_,
                    request[0],
                    start,
...
def version():
    """Print the version"""
    printer('internet_speed_tester %s' % __version__)
...
def shell():
    ...
    result_filepath = os.path.join(user_home, 'internetSpeed_results.csv')
    file_obj = open(result_filepath, 'a')
    file_header = "Date\t\t\tTime\t\tDownload[Mbits/s]\t\tUpload[Mbits/s]\t\tServer url\t\t\t\t\t\t\tLatency[ms]\n\n"
    download_speed = str(0.0)
    if os.path.getsize(result_filepath) == 0: # if file is empty, print the header first
    ...
    printer('Actual best server hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: Latency '
    ...
    if args.download:
        ...
        download_speed = str( round((results.download / 1000.0 / 1000.0) / args.units[1], 12) )
    else:
    ...
    if args.upload:
        ...
        upload_speed = str( round((results.upload / 1000.0 / 1000.0) / args.units[1], 9) )
        msg = date + "\t\t" + time + "\t" + download_speed + "\t\t\t" + upload_speed
        pos = results.server['url'].find(":", 5)
        server_url = '{:30s}'.format(results.server['url'][:pos])
        msg += "\t\t\t" + server_url + "\t\t" + str(results.server['latency']) + "\n"
        file_obj.write(msg)
    else:
